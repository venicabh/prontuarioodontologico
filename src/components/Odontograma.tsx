  ...
  return (
    <Popover open={open} onOpenChange={(o) => !disabled && setOpen(o)}>
      <PopoverTrigger asChild>
        ...
        >
          <span className="text-[10px] font-medium text-emerald-200 group-hover:text-white">
            {num}
          </span>
          <svg
            viewBox="0 0 40 58"
          ...
          >
            ...
            {state === "extracao" && (
              <>
                <line x1="6" y1="6" x2="34" y2="52" stroke="#7f1d1d" strokeWidth="2.5" strokeLinecap="round" />
                <line x1="34" y1="6" x2="6" y2="52" stroke="#7f1d1d" strokeWidth="2.5" strokeLinecap="round" />
              </>
            )}
          </svg>
        </button>
...
export function Odontograma({ value = {}, onChange, disabled }: Props) {
  ...
  return (
    <div className="space-y-4">
      <div className="rounded-lg border-2 border-emerald-800 bg-emerald-900 p-4 overflow-x-auto">
        <div className="flex flex-col items-center gap-3 min-w-fit">
          <div className="flex items-end gap-3">
            {row(UPPER_RIGHT, true)}
            <div className="w-px h-10 bg-emerald-600" />
            {row(UPPER_LEFT, true)}
          </div>
          <div className="h-px w-full bg-emerald-600" />
